...
// Only OpenAI-compatible provider is needed for local Ollama instances
...
export namespace Provider {
  ...
  // No custom loaders needed for local Ollama instances
  ...
  export async function getSmallModel(providerID: string) {
    ...
    // Return the first available model from the provider
    const provider = await state().then((state) => state.providers[providerID])
    if (provider) {
      const modelIDs = Object.keys(provider.models)
      if (modelIDs.length > 0) {
        return getModel(providerID, modelIDs[0])
      }
    }
